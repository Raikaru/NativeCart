...
static const FireredRomU32TableProfileRow s_battle_script_ptr_overlay_profile_rows[] = {
    { 0u, "__firered_builtin_battle_script_ptr_overlay_profile_none__", 0u },
};

static int gba_battle_script_rom_ptr_in_loaded_rom(u32 addr)
{
    size_t rom_size;
    size_t off;
...
const u8 *firered_portable_rom_battle_script_pointer_overlay_ptr(u32 token_index)
{
    ...
    if (token_index < FIRERED_PORTABLE_BATTLE_SCRIPT_ROM_OVERLAY_FIRST_INDEX)
        return NULL;

    n = (size_t)gFireredPortableBattleScriptPtrCount;
    if (n == 0u || (size_t)token_index >= n)
        return NULL;

    if (!firered_portable_rom_u32_table_resolve_base_off("FIRERED_ROM_BATTLE_SCRIPT_PTRS_OFF",
            s_battle_script_ptr_overlay_profile_rows, NELEMS(s_battle_script_ptr_overlay_profile_rows),
            &table_off))
        return NULL;
    ...
    if (!gba_battle_script_rom_ptr_in_loaded_rom(addr))
        return NULL;
