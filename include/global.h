...
#if defined(__APPLE__) || defined(__CYGWIN__) || defined(__INTELLISENSE__) || defined(PORTABLE_FAKE_INCBIN)
#define INCBIN(...) {0}
...
#define SWAP(a, b, temp) GBA_SWAP(a, b, temp)
...
// min/max macros - only defined for C code to avoid conflicts with C++ std::min/max
#ifndef __cplusplus
