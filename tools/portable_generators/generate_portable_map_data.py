...
# - map_object_event_scripts_portable_data.c (exact map object script pointer table for save/resolve)
...
def collect_map_object_script_symbols(maps):
    """Unique script symbol names from non-clone object events (for portable token table)."""
    syms = set()
    for map_data in maps.values():
        for event in map_data.get('object_events', []):
            ...
            if 'script' not in event:
                continue
            script = event['script']
            if script in (None, '0x0', 'NULL'):
                continue
            syms.add(script)
    return sorted(syms)


def write_map_object_event_scripts_portable_data(generated_dir, symbols):
    path = generated_dir / 'src' / 'data' / 'map_object_event_scripts_portable_data.c'
    out = [
    ...
    for sym in symbols:
        out.append(f'extern const u8 {sym}[];')
    out.append('')
    out.append('const void *const gFireredPortableMapObjectEventScriptPtrs[] = {')
    for sym in symbols:
        out.append(f'    {sym},')
    out.append('};')
    out.append('')
    out.append(
        'const u32 gFireredPortableMapObjectEventScriptPtrCount = '
        '(u32)(sizeof(gFireredPortableMapObjectEventScriptPtrs) / sizeof(gFireredPortableMapObjectEventScriptPtrs[0]));'
    )
    out.append('')
...
def append_object_events(out, map_name, events, map_indices):
    """
    ObjectEventTemplate.script is u32 on PORTABLE; C forbids (u32)(uintptr_t)sym in static
    initializers. Emit parallel const u8* table (valid constants) and .script = 0; runtime
    fixup in firered_portable_init_map_object_event_script_words().
    """
    script_label = f's{map_name}_ObjectEventScriptSrc'
    events_label = f's{map_name}_ObjectEvents'
    n = len(events)

    out.append(f'static const u8 * const {script_label}[] = {{')
    for index, event in enumerate(events, start=1):
        require_fields(event, ('type', 'graphics_id', 'x', 'y'), f'{map_name} object_event[{index}]')
        if event['type'] == 'clone':
            out.append('    NULL,')
        else:
            require_fields(event, ('script',), f'{map_name} object_event[{index}]')
            out.append(f'    {c_script_ptr(event["script"])},')
    out.append('};')
    out.append('')

    out.append(f'static struct ObjectEventTemplate {events_label}[] = {{')
    for index, event in enumerate(events, start=1):
        if event['type'] == 'clone':
    ...
    out.append('};')
    out.append('')
    return events_label, script_label, n
...
def generate_source(root_dir):
    ...
    for map_name in sorted(maps):
        ...
        if object_events:
            object_label, object_script_src_label, object_event_count = append_object_events(out, map_name, object_events, map_indices)
            object_event_fixups.append((object_label, object_script_src_label, object_event_count))
        else:
            object_label = 'NULL'
    ...
    out.append('};')
    out.append('')
    out.append(f'const u32 gMapLayoutsSlotCount = {len(layouts)}u;')
    out.append('')
    out.append('const u8 gMapGroupSizes[MAP_GROUPS_COUNT] = {')
    for group_name in map_groups['group_order']:
        out.append(f'    {len(map_groups[group_name])}u,')
    out.append('};')
    out.append('')
    ...
    out.append('{')
    if object_event_fixups:
        out.append('    u32 i;')
        for ev_label, src_label, n in object_event_fixups:
            out.append(f'    for (i = 0; i < (u32){n}; i++)')
            out.append(
                f'        {ev_label}[i].script = '
                f'firered_portable_map_object_event_script_ptr_to_save_u32({src_label}[i]);'
            )
    else:
        out.append('    (void)0;')
    out.append('}')
...
def main(argv):
    ...
    maps = load_maps(root_dir)
    write_map_object_event_scripts_portable_data(
        generated_dir, collect_map_object_script_symbols(maps)
    )
