...
ifeq ($(OS),Windows_NT)
  # Prefer the MinGW toolchain directory ahead of Git-for-Windows DLLs.
  HOST_GCC := $(firstword $(shell where gcc 2>NUL))
  HOST_MINGW_MAKE := $(firstword $(shell where mingw32-make 2>NUL))
  MINGW_BINDIR := $(strip $(if $(HOST_GCC),$(dir $(HOST_GCC)),$(if $(HOST_MINGW_MAKE),$(dir $(HOST_MINGW_MAKE)))))
  ifneq ($(MINGW_BINDIR),)
...
ifeq ($(SETUP_PREREQS),1)
  $(foreach line, $(shell status=0; logfile=.make.check_root_deps.log; $(MAKE) check-root-deps NODEP=1 SETUP_PREREQS=0 > $$logfile 2>&1 || status=$$?; sed "s/ /__SPACE__/g" $$logfile; rm -f $$logfile; exit $$status), $(info $(subst __SPACE__, ,$(line))))
  ifneq ($(.SHELLSTATUS),0)
    $(error Missing required root build dependencies. See diagnostics above for exact missing tools and libraries)
  endif
...
ifeq ($(OS),Windows_NT)
LD_RESPONSE_FILE := $(OBJ_DIR)/ld.rsp
$(LD_RESPONSE_FILE): Makefile $(OBJS)
	$(file >$@,$(OBJS_REL) $(LIB))
else
LD_RESPONSE_FILE :=
endif
...
check-root-deps:
	@status=0; \
	$(MAKE) -f make_tools.mk check-tools-deps || status=1; \
	$(MAKE) check-agbcc NODEP=1 SETUP_PREREQS=0 || status=1; \
	$(MAKE) check-arm-toolchain NODEP=1 SETUP_PREREQS=0 || status=1; \
	exit $$status

check-agbcc:
ifeq ($(MODERN),0)
	@status=0; \
	for tool in "$(TOOLS_DIR)/agbcc/bin/agbcc$(EXE)" "$(TOOLS_DIR)/agbcc/bin/old_agbcc$(EXE)" "$(TOOLS_DIR)/agbcc/bin/agbcc_arm$(EXE)"; do \
		if [ ! -f "$$tool" ]; then \
			echo "ERROR: missing required agbcc binary: $$tool"; \
			status=1; \
		fi; \
	done; \
	if [ $$status -ne 0 ]; then \
		echo "This root build uses the non-modern compiler path (MODERN=$(MODERN))."; \
		echo "Checked paths under: $(TOOLS_DIR)/agbcc/bin"; \
		echo "Windows hint: build repo tools so $(TOOLS_DIR)/agbcc/bin/*.exe exist."; \
		echo "Build/install agbcc so the listed binaries exist, then rerun make."; \
		exit $$status; \
	fi
else
	@:
...
check-arm-toolchain:
	@command -v "$(AS)" >/dev/null 2>&1 || { \
		status=$$?; \
		echo "ERROR: missing required assembler command: $(AS)"; \
		echo "Checked command via PATH lookup: $(AS)"; \
		if [ -n "$(TOOLCHAIN)" ]; then \
			echo "Configured toolchain directory: $(TOOLCHAIN)/bin"; \
		fi; \
		echo "Windows hint: install devkitARM or MSYS2 arm-none-eabi binutils."; \
		echo "Install devkitARM/binutils so $(AS) is available on PATH, then rerun make."; \
		exit $$status; \
	}
...
LDFLAGS = -Map ../../$(MAP)
$(ELF): $(LD_SCRIPT) $(LD_SCRIPT_DEPS) $(OBJS) $(LD_RESPONSE_FILE)
	@cd $(OBJ_DIR) && $(LD_ENV) $(LD) $(LDFLAGS) -T ../../$< --print-memory-usage -o ../../$@ $(if $(LD_RESPONSE_FILE),@ld.rsp,$(OBJS_REL) $(LIB)) | cat
	@echo "cd $(OBJ_DIR) && $(LD_ENV) $(LD) $(LDFLAGS) -T ../../$< --print-memory-usage -o ../../$@ $(if $(LD_RESPONSE_FILE),@ld.rsp,<objs> <libs>) | cat"
