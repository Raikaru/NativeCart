...
/* Avoid large `struct Pokemon` on stack — agbcc can ICE in stor-layout.c. */
static EWRAM_DATA struct Pokemon sDaycarePokemonBuffer;
...
static u16 TakeSelectedPokemonFromDaycare(struct DaycareMon *daycareMon)
{
    u16 species;
    u32 experience;
    struct Pokemon *pokemon = &sDaycarePokemonBuffer;
...
static u16 DetermineEggSpeciesAndParentSlots(struct DayCare *daycare, u8 *parentSlots)
{
    ...
    eggSpecies = Daycare_GetEggSpecies(species[parentSlots[0]]);
...
{
    struct Pokemon *egg = &sDaycarePokemonBuffer;
    u16 species;
    ...
    Daycare_BuildEggMoveset(egg, &daycare->mons[parentSlots[1]].mon, &daycare->mons[parentSlots[0]].mon);
