...
#define gBagBgPalette gBagBgPalette_Portable
#define gBagBgPalette_FemaleOverride gBagBgPalette_FemaleOverride_Portable

extern void firered_runtime_trace_external(const char *message);

static void TraceBagLoadState(const char *prefix, int state)
{
    char buffer[96];

    snprintf(buffer, sizeof(buffer), "%s%d", prefix, state);
...
#ifdef PORTABLE
static const u16 *const sItemListTilemap = sItemListTilemap_Portable;
#else
...
static bool8 LoadBagMenuGraphics(void)
{
    ...
    if (sLastMainState != gMain.state)
    {
        sLastMainState = gMain.state;
        TraceBagLoadState("LoadBagMenuGraphics: state=", gMain.state);
    }
#endif
...
{
#ifdef PORTABLE
    static int sLastBagGfxState = -1;
    if (sLastBagGfxState != sBagMenuDisplay->data[0])
    {
        sLastBagGfxState = sBagMenuDisplay->data[0];
        TraceBagLoadState("DoLoadBagGraphics: state=", sBagMenuDisplay->data[0]);
    }
#endif
