...
static bool8 ShouldFastCompleteBattleIntroTask(u8 taskId)
{
    return gMain.callback2 == BattleMainCB2
        && (gIntroSlideFlags & 1) == 0
        && gTasks[taskId].data[0] >= 2;
}

static void FastCompleteBattleIntroTask(u8 taskId)
{
...
{
#ifdef PORTABLE
    TraceBattleIntro("BattleIntro: end task=%u", taskId);
#endif
    DestroyTask(taskId);
#ifdef PORTABLE
    TraceBattleIntro("BattleIntro: end post-DestroyTask");
#endif
    gIntroSlideFlags &= ~1;
    gBattle_BG1_X = 0;
    gBattle_BG1_Y = 0;
    ...
    gBattle_BG2_Y = 0;
#ifdef PORTABLE
    TraceBattleIntro("BattleIntro: end post-bg-reset");
#endif
    SetGpuReg(REG_OFFSET_BLDCNT, 0);
    SetGpuReg(REG_OFFSET_BLDALPHA, 0);
    SetGpuReg(REG_OFFSET_BLDY, 0);
#ifdef PORTABLE
    TraceBattleIntro("BattleIntro: end post-blend-reset");
#endif
    SetGpuReg(REG_OFFSET_WININ, WININ_WIN0_BG_ALL | WININ_WIN0_OBJ | WININ_WIN0_CLR | WININ_WIN1_BG_ALL | WININ_WIN1_OBJ | WININ_WIN1_CLR);
    SetGpuReg(REG_OFFSET_WINOUT, WINOUT_WIN01_BG_ALL | WINOUT_WIN01_OBJ | WINOUT_WIN01_CLR | WINOUT_WINOBJ_BG_ALL | WINOUT_WINOBJ_OBJ | WINOUT_WINOBJ_CLR);
#ifdef PORTABLE
    TraceBattleIntro("BattleIntro: end post-win-reset");
#endif
}
...
#ifdef PORTABLE
    TraceBattleIntro("BattleIntro: slide1 task=%u state=%d arg1=%d arg2=%d arg3=%d", taskId, gTasks[taskId].data[0], gTasks[taskId].data[1], gTasks[taskId].data[2], gTasks[taskId].data[3]);
    if (ShouldFastCompleteBattleIntroTask(taskId))
    {
        FastCompleteBattleIntroTask(taskId);
        return;
    }
#endif
...
#ifdef PORTABLE
    TraceBattleIntro("BattleIntro: slide2 task=%u state=%d arg1=%d arg2=%d arg3=%d", taskId, gTasks[taskId].data[0], gTasks[taskId].data[1], gTasks[taskId].data[2], gTasks[taskId].data[3]);
    if (ShouldFastCompleteBattleIntroTask(taskId))
    {
        FastCompleteBattleIntroTask(taskId);
        return;
...
#ifdef PORTABLE
    TraceBattleIntro("BattleIntro: slide3 task=%u state=%d arg1=%d arg2=%d arg3=%d", taskId, gTasks[taskId].data[0], gTasks[taskId].data[1], gTasks[taskId].data[2], gTasks[taskId].data[3]);
    if (ShouldFastCompleteBattleIntroTask(taskId))
    {
        FastCompleteBattleIntroTask(taskId);
        return;
...
#ifdef PORTABLE
    TraceBattleIntro("BattleIntro: slideLink task=%u state=%d arg1=%d arg2=%d", taskId, gTasks[taskId].data[0], gTasks[taskId].data[1], gTasks[taskId].data[2]);
    if (ShouldFastCompleteBattleIntroTask(taskId))
    {
        FastCompleteBattleIntroTask(taskId);
        return;
