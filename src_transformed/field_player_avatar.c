...
{
#ifdef PORTABLE
    static bool8 sLoggedPlayerAvatarTransition;

    if (!sLoggedPlayerAvatarTransition)
    {
        char buffer[96];

        snprintf(buffer, sizeof(buffer), "UpdatePlayerAvatarTransitionState: obj=%u flags=%02X",
                 gPlayerAvatar.objectEventId, gPlayerAvatar.flags);
        firered_runtime_trace_external(buffer);
        sLoggedPlayerAvatarTransition = TRUE;
    }
#endif
