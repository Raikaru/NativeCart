...
static const u8 *ResolveScriptDataPointer(u32 value)
{
    if (value == 0)
        return NULL;
    return (const u8 *)firered_portable_resolve_script_ptr(value);
}
#endif
...
{
    bool8 (*func)(void) = (bool8 (*)(void))ScriptReadPtr(ctx);
...
{
    void (*func )(void) = ((void (*)(void))ScriptReadPtr(ctx));
    func();
    return FALSE;
}
...
bool8 ScrCmd_copybyte(struct ScriptContext * ctx)
{
    ...
    *dest = *(const u8 *)ScriptReadPtr(ctx);
    return FALSE;
}
...
{
    const u16 varId = ScriptReadHalfword(ctx);
    const u16 value2 = ScriptReadHalfword(ctx);
    const u16 *ptr = GetVarPointer(varId);
    const u16 value1 = (ptr != NULL) ? *ptr : varId;
...
#ifdef PORTABLE
    movementScript = ResolveScriptOperandPointer(movementScript);
#endif
...
bool8 ScrCmd_message(struct ScriptContext * ctx)
{
    ...
    if (msg == NULL)
        msg = ResolveScriptDataPointer(ctx->data[0]);
#ifdef PORTABLE
    {
        unsigned long frame = firered_runtime_get_completed_frame_external();
        if (frame >= 1970 && frame <= 1975)
        {
            char buffer[160];
            snprintf(buffer, sizeof(buffer),
                     "SCRIPT MSG frame=%lu msg=%p data0=%08lX",
                     frame,
                     msg,
...
bool8 ScrCmd_braillemessage(struct ScriptContext * ctx)
{
    ...
    if (msg == NULL)
        msg = (u8 *)ResolveScriptDataPointer(ctx->data[0]);
