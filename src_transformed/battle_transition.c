...
#ifdef PORTABLE
    TraceBattleTransition("BattleTrans: launch id=%u task=%u", transitionId, taskId);
#endif
...
{
#ifdef PORTABLE
    TraceBattleTransition("BattleTrans: task state=%d transition=%d", gTasks[taskId].tState, gTasks[taskId].tTransitionId);
#endif
...
{
#ifdef PORTABLE
    TraceBattleTransition("BattleTrans: start intro transition=%d", task->tTransitionId);
#endif
...
{
#ifdef PORTABLE
    TraceBattleTransition("BattleTrans: wait intro transition=%d", task->tTransitionId);
#endif
...
{
#ifdef PORTABLE
    TraceBattleTransition("BattleTrans: start main transition=%d func=%p", task->tTransitionId, sTasks_Main[task->tTransitionId]);
#endif
...
#ifdef PORTABLE
    TraceBattleTransition("BattleTrans: wait main transition=%d done=%d", task->tTransitionId, task->tTransitionDone);
#endif
    return FALSE;
}
...
{
#ifdef PORTABLE
    TraceBattleTransition("BattleTrans: pokeballs state=%d", gTasks[taskId].tState);
#endif
...
static bool8 PokeballsTrail_Init(struct Task *task)
{
    ...
    task->tState++;
#ifdef PORTABLE
    TraceBattleTransition("BattleTrans: pokeballs init");
#endif
    return FALSE;
}
...
static bool8 PokeballsTrail_Main(struct Task *task)
{
    ...
    task->tState++;
#ifdef PORTABLE
    TraceBattleTransition("BattleTrans: pokeballs main spawned");
#endif
    return FALSE;
}
...
{
#ifdef PORTABLE
    TraceBattleTransition("BattleTrans: angled state=%d wipe=%d delay=%d", gTasks[taskId].tState, gTasks[taskId].tWipeId, gTasks[taskId].tDelay);
#endif
...
static bool8 AngledWipes_Init(struct Task *task)
{
    ...
    task->tState++;
#ifdef PORTABLE
    TraceBattleTransition("BattleTrans: angled init");
#endif
    return TRUE;
}
...
static bool8 AngledWipes_SetWipeData(struct Task *task)
{
    ...
    task->tState++;
#ifdef PORTABLE
    TraceBattleTransition("BattleTrans: angled set wipe=%d dir=%d", task->tWipeId, task->tDir);
#endif
    return TRUE;
}
...
static bool8 AngledWipes_DoWipe(struct Task *task)
{
    ...
    sTransitionData->vblankDma++;
#ifdef PORTABLE
    TraceBattleTransition("BattleTrans: angled do wipe=%d curr=(%d,%d)", task->tWipeId, sTransitionData->tWipeCurrX, sTransitionData->tWipeCurrY);
#endif
    return FALSE;
}
...
#ifdef PORTABLE
        TraceBattleTransition("BattleTrans: angled next wipe=%d delay=%d", task->tWipeId, task->tDelay);
#endif
        return TRUE;
    }
    else
...
#ifdef PORTABLE
        TraceBattleTransition("BattleTrans: angled end all");
#endif
        return FALSE;
    }
}
...
static bool8 AngledWipes_StartNext(struct Task *task)
{
    if (--task->tDelay == 0)
    {
        ...
        task->tState = 1;
#ifdef PORTABLE
        TraceBattleTransition("BattleTrans: angled restart wipe=%d", task->tWipeId);
#endif
        return TRUE;
    }
    else
