...
#define PORTABLE_BOX_NAMES_OFFSET 0x8344
#define PORTABLE_BOX_WALLPAPERS_OFFSET 0x83C2

static const u8 sPortableBoxNameText[] = {CHAR_B, CHAR_O, CHAR_X, EOS};

static void EnsurePokemonStoragePointerPortable(void)
{
    uintptr_t storageBase = (uintptr_t)&gPokemonStorage;
    uintptr_t storageLimit = storageBase + sizeof(gPokemonStorage);
    uintptr_t storagePtr = (uintptr_t)gPokemonStoragePtr;

    if (storagePtr < storageBase || storagePtr >= storageLimit)
    {
        uintptr_t saveBase = (uintptr_t)&gSaveBlock1;
        uintptr_t saveLimit = saveBase + sizeof(gSaveBlock1);
        uintptr_t savePtr = (uintptr_t)gSaveBlock1Ptr;
        uintptr_t offset = 0;

        if (savePtr >= saveBase && savePtr <= saveLimit)
            offset = savePtr - saveBase;

        gPokemonStoragePtr = (void *)(storageBase + offset);
        firered_runtime_trace_external("ResetPokemonStorageSystem: repaired gPokemonStoragePtr");
    }
}
#endif
...
void DrawTextWindowAndBufferTiles(const u8 *string, void *dst, u8 zero1, u8 zero2, u8 *unused, s32 bytesToBuffer)
{
    ...
    tileData1 = GetWindowTileDataAddress(windowId);
...
void ResetPokemonStorageSystem(void)
{
    ...
    {
        char buffer[256];
        uintptr_t storageBase = (uintptr_t)gPokemonStoragePtr;
        uintptr_t boxNamesPtr = (uintptr_t)GetBoxNamePtr(0);
        uintptr_t boxWallsPtr = (uintptr_t)&gPokemonStoragePtr->boxWallpapers[0];

        snprintf(buffer, sizeof(buffer),
                 "ResetPokemonStorageSystem: enter sav1=%p storage=%p base=%p size=%lu box0=%p ofs=%lu exp=%u wall0=%p wall_ofs=%lu wall_exp=%u",
                 gSaveBlock1Ptr,
                 gPokemonStoragePtr,
                 &gPokemonStorage,
                 (unsigned long)sizeof(*gPokemonStoragePtr),
                 (void *)boxNamesPtr,
                 (unsigned long)(boxNamesPtr - storageBase),
                 PORTABLE_BOX_NAMES_OFFSET,
                 (void *)boxWallsPtr,
                 (unsigned long)(boxWallsPtr - storageBase),
                 PORTABLE_BOX_WALLPAPERS_OFFSET);
    ...
    SetCurrentBox(0);
#ifdef PORTABLE
    firered_runtime_trace_external("ResetPokemonStorageSystem: post-SetCurrentBox");
    memset(gPokemonStoragePtr, 0, sizeof(*gPokemonStoragePtr));
    firered_runtime_trace_external("ResetPokemonStorageSystem: post-memset-storage");
#else
    ...
    for (boxId = 0; boxId < TOTAL_BOXES_COUNT; boxId++)
    {
        u8 *namePtr = GetBoxNamePtr(boxId);
        u8 *dest;
        ...
        if (boxId < 2)
        {
            char buffer[192];
            uintptr_t storageBase = (uintptr_t)gPokemonStoragePtr;
            uintptr_t storageEnd = storageBase + sizeof(*gPokemonStoragePtr);
            uintptr_t destPtr = (uintptr_t)namePtr;

            snprintf(buffer, sizeof(buffer),
                     "ResetPokemonStorageSystem: pre-copy box=%u name=%p src=%p in_range=%u ofs=%lu",
                     boxId,
                     (void *)destPtr,
                     gText_Box,
                     destPtr >= storageBase && destPtr < storageEnd,
                     (unsigned long)(destPtr - storageBase));
            firered_runtime_trace_external(buffer);
        }
#endif
        dest = StringCopy(namePtr, sPortableBoxNameText);
#ifdef PORTABLE
        if (boxId < 2)
            firered_runtime_trace_external("ResetPokemonStorageSystem: post-StringCopy");
#endif
        ConvertIntToDecimalStringN(dest, boxId + 1, STR_CONV_MODE_LEFT_ALIGN, 2);
#ifdef PORTABLE
        if (boxId < 2)
            firered_runtime_trace_external("ResetPokemonStorageSystem: post-ConvertIntToDecimalStringN");
#endif
    }
...
#ifdef PORTABLE
        if (boxId < 2)
            firered_runtime_trace_external("ResetPokemonStorageSystem: pre-SetBoxWallpaper");
#endif
