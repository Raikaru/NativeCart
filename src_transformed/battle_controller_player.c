...
static void Intro_TryShinyAnimShowHealthbox(void)
{
    if (!gBattleSpritesDataPtr->healthBoxesData[gActiveBattler].ballAnimActive && !gBattleSpritesDataPtr->healthBoxesData[gActiveBattler ^ BIT_FLANK].ballAnimActive)
    {
        ...
        {
            if (IsPortableLiveSprite(gBattleControllerData[gActiveBattler ^ BIT_FLANK]))
...
static void Task_GiveExpToMon(u8 taskId)
{
    ...
    if (IsDoubleBattle() == TRUE || monId != gBattlerPartyIndexes[battlerId]) // Give exp without moving the expbar.
    {
        ...
        u32 nextLvlExp = ExperienceTableGet(gSpeciesInfo[species].growthRate, level + 1);
...
static void Task_PrepareToGiveExpWithExpBar(u8 taskId)
{
    ...
    u32 currLvlExp = ExperienceTableGet(gSpeciesInfo[species].growthRate, level);
    u32 expToNextLvl;

    exp -= currLvlExp;
    expToNextLvl = ExperienceTableGet(gSpeciesInfo[species].growthRate, level + 1) - currLvlExp;
...
static void Task_GiveExpWithExpBar(u8 taskId)
{
    ...
    else
    {
        ...
        if (newExpPoints == -1) // The bar has been filled with given exp points.
        {
            ...
            expOnNextLvl = ExperienceTableGet(gSpeciesInfo[species].growthRate, level + 1);
...
#ifdef PORTABLE
    {
        u8 bits = gBattleBufferA[gActiveBattler][2];
        int count = 0;
        int b;
        ...
        if (count > 2)
        {
            ...
            for (b = 0; b < 6; b++)
            {
                if (bits & (1 << b))
                {
                    seen++;
                    if (seen > 2)
                        gBattleBufferA[gActiveBattler][2] &= ~(1 << b);
                }
            }
        }
    }
...
static void PlayerHandleDMA3Transfer(void)
{
    uintptr_t dstArg = gBattleBufferA[gActiveBattler][1]
