...
#ifdef PORTABLE
static const char *const sPortableMonAssetPrefixes[] = {
    "",
    "../",
    "../../",
    "../../../",
...
static const u32 *sPortableMonFrontPicData[SPECIES_UNOWN_QMARK + 1];
static const u32 *sPortableMonBackPicData[SPECIES_UNOWN_QMARK + 1];
static const u32 *sPortableMonPaletteData[SPECIES_UNOWN_QMARK + 1];
static const u32 *sPortableMonShinyPaletteData[SPECIES_UNOWN_QMARK + 1];
static struct CompressedSpritePalette sPortableMonPaletteStructs[SPECIES_UNOWN_QMARK + 1];
static struct CompressedSpritePalette sPortableMonShinyPaletteStructs[SPECIES_UNOWN_QMARK + 1];

static u16 NormalizePortableMonSpecies(u16 species, const char *const *pathTable)
{
    if (species <= SPECIES_UNOWN_QMARK && pathTable[species] != NULL)
        return species;
    return SPECIES_NONE;
}

static void *LoadPortableMonAssetFile(const char *relativePath)
{
    FILE *file;
    void *data;
    char fullPath[512];
    ...
    assetRoot = getenv("FIRERED_ASSET_ROOT");
    if (assetRoot != NULL && assetRoot[0] != '\0')
    {
        size_t rl = strlen(assetRoot);

        if (rl + strlen(relativePath) + 2 < sizeof(fullPath))
        {
            memcpy(fullPath, assetRoot, rl);
            if (rl > 0 && (assetRoot[rl - 1] == '/' || assetRoot[rl - 1] == '\\'))
                snprintf(fullPath + rl, sizeof(fullPath) - rl, "%s", relativePath);
            else
                snprintf(fullPath + rl, sizeof(fullPath) - rl, "/%s", relativePath);
    ...
    {
        snprintf(fullPath, sizeof(fullPath), "%s%s", sPortableMonAssetPrefixes[i], relativePath);
        file = fopen(fullPath, "rb");
        if (file != NULL)
            break;
    }

    if (i == ARRAY_COUNT(sPortableMonAssetPrefixes))
        return NULL;
...
static const u32 *GetPortableMonCompressedData(const char *const *pathTable, const u32 **cache, u16 species)
{
    species = NormalizePortableMonSpecies(species, pathTable);
    if (cache[species] == NULL)
    {
        void *loaded = LoadPortableMonAssetFile(pathTable[species]);
...
const u32 *GetPortableMonPicData(u16 species, bool8 isFrontPic)
{
    if (isFrontPic)
        return GetPortableMonCompressedData(sMonFrontPicPaths_Portable, sPortableMonFrontPicData, species);
    else
        return GetPortableMonCompressedData(sMonBackPicPaths_Portable, sPortableMonBackPicData, species);
}

const u32 *GetPortableMonPaletteData(u16 species, bool8 shiny)
{
    if (shiny)
        return GetPortableMonCompressedData(sMonShinyPalettePaths_Portable, sPortableMonShinyPaletteData, species);
    else
        return GetPortableMonCompressedData(sMonPalettePaths_Portable, sPortableMonPaletteData, species);
}

const struct CompressedSpritePalette *GetPortableMonSpritePalStruct(u16 species, bool8 shiny)
{
    u16 normalizedSpecies;

    if (shiny)
    {
        normalizedSpecies = NormalizePortableMonSpecies(species, sMonShinyPalettePaths_Portable);
        sPortableMonShinyPaletteStructs[normalizedSpecies].data = GetPortableMonPaletteData(normalizedSpecies, TRUE);
        sPortableMonShinyPaletteStructs[normalizedSpecies].tag = gMonShinyPaletteTable[normalizedSpecies].tag;
        return &sPortableMonShinyPaletteStructs[normalizedSpecies];
    }
    else
    {
        normalizedSpecies = NormalizePortableMonSpecies(species, sMonPalettePaths_Portable);
        sPortableMonPaletteStructs[normalizedSpecies].data = GetPortableMonPaletteData(normalizedSpecies, FALSE);
        sPortableMonPaletteStructs[normalizedSpecies].tag = gMonPaletteTable[normalizedSpecies].tag;
        return &sPortableMonPaletteStructs[normalizedSpecies];
    }
}
#endif
...
const u32 *GetMonSpritePalFromSpeciesAndPersonality(u16 species, u32 otId, u32 personality)
{
    ...
    if (shinyValue < SHINY_ODDS)
        return GetPortableMonPaletteData(species, TRUE);
    else
        return GetPortableMonPaletteData(species, FALSE);
#else
    if (species > SPECIES_EGG)
...
const struct CompressedSpritePalette *GetMonSpritePalStructFromOtIdPersonality(u16 species, u32 otId , u32 personality)
{
    ...
    if (shinyValue < SHINY_ODDS)
        return GetPortableMonSpritePalStruct(species, TRUE);
    else
        return GetPortableMonSpritePalStruct(species, FALSE);
#else
    if (shinyValue < SHINY_ODDS)
