...
static const struct TrainerBattleParameter sEarlyRivalBattleParams[] =
{
    ...
    {&sTrainerAIntroSpeech,         TRAINER_PARAM_CLEAR_VAL_32BIT},
    {&sTrainerADefeatSpeech,        TRAINER_PARAM_LOAD_PTR},
    {&sTrainerVictorySpeech,        TRAINER_PARAM_LOAD_PTR},
    {&sTrainerCannotBattleSpeech,   TRAINER_PARAM_CLEAR_VAL_32BIT},
    {&sTrainerABattleScriptRetAddr, TRAINER_PARAM_CLEAR_VAL_32BIT},
    {&sTrainerBattleEndScript,      TRAINER_PARAM_LOAD_SCRIPT_RET_ADDR},
};
...
static void Task_BattleStart(u8 taskId)
{
...
#ifdef PORTABLE
    TraceBattleSetup("BattleStart: task=%u state=%d transition=%d", taskId, tState, tTransition);
#endif
    switch (tState)
    {
    case 0:
...
#ifdef PORTABLE
            TraceBattleSetup("BattleStart: started transition=%d", tTransition);
#endif
            ++tState;
        }
        break;
    case 1:
        if (IsBattleTransitionDone() == TRUE)
        {
            HelpSystem_Enable();
#ifdef PORTABLE
            TraceBattleSetup("BattleStart: pre-cleanup");
#endif
            CleanupOverworldWindowsAndTilemaps();
#ifdef PORTABLE
            TraceBattleSetup("BattleStart: transition done, CB2_InitBattle next");
#endif
            SetMainCallback2(CB2_InitBattle);
#ifdef PORTABLE
            TraceBattleSetup("BattleStart: post-SetMainCallback2");
#endif
            RestartWildEncounterImmunitySteps();
#ifdef PORTABLE
            TraceBattleSetup("BattleStart: post-RestartWildEncounterImmunitySteps");
#endif
            ClearPoisonStepCounter();
#ifdef PORTABLE
            TraceBattleSetup("BattleStart: post-ClearPoisonStepCounter");
#endif
            DestroyTask(taskId);
#ifdef PORTABLE
            TraceBattleSetup("BattleStart: post-DestroyTask");
#endif
        }
        break;
    }
...
#ifdef PORTABLE
    TraceBattleSetup("BattleStart: create task=%u transition=%u song=%u", taskId, transition, song);
#endif
...
static u8 GetTrainerBattleTransition(void)
{
    ...
    if (enemyLevel < playerLevel)
        transition = sBattleTransitionTable_Trainer[transitionType][0];
    else
        transition = sBattleTransitionTable_Trainer[transitionType][1];
#ifdef PORTABLE
    transition = B_TRANSITION_SLICE;
#endif
    return transition;
}
...
#ifdef PORTABLE
    TraceBattleSetup("BattleStart: StartTrainerBattle mode=%u rivalFlags=%u opponent=%u typeFlags=%08X", GetTrainerBattleMode(), GetRivalBattleFlags(), gTrainerBattleOpponent_A, gBattleTypeFlags);
#endif
