...
bool16 InitWindows(const struct WindowTemplate *templates)
{
    ...
    for (i = 0, allocatedBaseBlock = 0, bgLayer = templates[i].bg; bgLayer != 0xFF && i < WINDOWS_MAX; ++i, bgLayer = templates[i].bg)
    {
        ...
        CopyWindowTemplate(&gWindows[i].window, &templates[i]);
...
u16 AddWindow(const struct WindowTemplate *template)
{
    ...
    CopyWindowTemplate(&gWindows[win].window, template);
...
{
    dest->bg = src->bg;
    dest->tilemapLeft = src->tilemapLeft;
    dest->tilemapTop = src->tilemapTop;
    dest->width = src->width;
    dest->height = src->height;
    dest->paletteNum = src->paletteNum;
    dest->baseBlock = src->baseBlock;
}
