...
void GetElevatorFloor(void)
{
    ...
    if (gSaveBlock1Ptr->dynamicWarp.mapGroup == MAP_GROUP(MAP_ROCKET_HIDEOUT_B1F))
    {
        ...
        case MAP_NUM(MAP_SILPH_CO_11F):
            floor = 14;
            break;
        // Note: Rocket Hideout cases removed - they duplicate Silph Co floor numbers
        // and this block only executes for Rocket Hideout map group anyway
        }
    }
...
static bool8 HasMonBeenRenamed(u8 idx)
{
    ...
    if (language != LANGUAGE_ENGLISH)
        return TRUE;
    GetSpeciesName(speciesName, GetMonData(pokemon, MON_DATA_SPECIES, NULL));
    if (StringCompare(speciesName, gStringVar1) != 0)
        return TRUE;
    else
        return FALSE;
}
...
static void ChangePokemonNickname_CB(void)
{
    ...
    firered_runtime_trace_external("FieldSpecials: ChangePokemonNickname_CB pre-SetMonData");
#endif
    SetMonData(&gPlayerParty[gSpecialVar_0x8004], MON_DATA_NICKNAME, gStringVar2);
#ifdef PORTABLE
    firered_runtime_trace_external("FieldSpecials: ChangePokemonNickname_CB post-SetMonData");
    firered_runtime_trace_external("FieldSpecials: ChangePokemonNickname_CB pre-return-field");
#endif
    CB2_ReturnToFieldContinueScriptPlayMapMusic();
#ifdef PORTABLE
    firered_runtime_trace_external("FieldSpecials: ChangePokemonNickname_CB post-return-field");
#endif
}
