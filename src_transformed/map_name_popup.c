...
static void Task_MapNamePopup(u8 taskId)
{
    ...
    case 0:
#ifdef PORTABLE
        firered_runtime_trace_external("MapNamePopup: state0 enter");
#endif
        task->tWindowId = MapNamePopupCreateWindow(task->tPalIntoFadedBuffer);
#ifdef PORTABLE
        firered_runtime_trace_external("MapNamePopup: state0 created-window");
#endif
...
#ifdef PORTABLE
    firered_runtime_trace_external("MapNamePopup: AddWindow done");
#endif
...
#ifdef PORTABLE
    firered_runtime_trace_external("MapNamePopup: palette done");
#endif
    LoadStdWindowTiles(windowId, tileNum);
#ifdef PORTABLE
    firered_runtime_trace_external("MapNamePopup: window tiles done");
#endif
    DrawTextBorderOuter(windowId, tileNum, WIN_PAL_NUM);
#ifdef PORTABLE
    firered_runtime_trace_external("MapNamePopup: border done");
#endif
    PutWindowTilemap(windowId);
#ifdef PORTABLE
    firered_runtime_trace_external("MapNamePopup: tilemap done");
#endif
    MapNamePopupPrintMapNameOnWindow(windowId);
#ifdef PORTABLE
    firered_runtime_trace_external("MapNamePopup: text done");
#endif
    CopyWindowToVram(windowId, COPYWIN_FULL);
#ifdef PORTABLE
    firered_runtime_trace_external("MapNamePopup: copy done");
#endif
    return windowId;
}
