...
static const u32 *sPortableTrainerFrontPicData[ARRAY_COUNT(sTrainerFrontPicPaths_Portable)];
static const u32 *sPortableTrainerFrontPaletteData[ARRAY_COUNT(sTrainerFrontPalettePaths_Portable)];

static void *LoadPortableBattleAssetFile(const char *relativePath)
{
    FILE *file;
    void *data;
    char fullPath[512];
    ...
    {
        snprintf(fullPath, sizeof(fullPath), "%s%s", sPortableBattleAssetPrefixes[i], relativePath);
        file = fopen(fullPath, "rb");
        if (file != NULL)
            break;
    }

    if (i == ARRAY_COUNT(sPortableBattleAssetPrefixes))
        return NULL;
...
static const u32 *GetPortableTrainerFrontPicData(u16 frontPicId)
{
    ...
    if (sPortableTrainerFrontPicData[frontPicId] == NULL)
        sPortableTrainerFrontPicData[frontPicId] = LoadPortableBattleAssetFile(sTrainerFrontPicPaths_Portable[frontPicId]);
...
static const u32 *GetPortableTrainerFrontPaletteData(u16 frontPicId)
{
    ...
    if (sPortableTrainerFrontPaletteData[frontPicId] == NULL)
        sPortableTrainerFrontPaletteData[frontPicId] = LoadPortableBattleAssetFile(sTrainerFrontPalettePaths_Portable[frontPicId]);
    return sPortableTrainerFrontPaletteData[frontPicId];
}
#endif

#ifdef PORTABLE
#define BATTLE_HEALTHBOX_PAL_DATA gBattleInterface_Healthbox_Pal_Portable
#define BATTLE_HEALTHBAR_PAL_DATA gBattleInterface_Healthbar_Pal_Portable
#define BATTLE_HEALTHBOX_SINGLES_PLAYER_GFX gHealthboxSinglesPlayerGfx_Portable
#define BATTLE_HEALTHBOX_SINGLES_OPPONENT_GFX gHealthboxSinglesOpponentGfx_Portable
#define BATTLE_HEALTHBOX_DOUBLES_PLAYER_GFX gHealthboxDoublesPlayerGfx_Portable
#define BATTLE_HEALTHBOX_DOUBLES_OPPONENT_GFX gHealthboxDoublesOpponentGfx_Portable
#define BATTLE_HEALTHBOX_SAFARI_GFX gHealthboxSafariGfx_Portable
#define BATTLE_HP_NUMBERS_GFX gInterfaceGfx_HPNumbers_Portable
#else
#define BATTLE_HEALTHBOX_PAL_DATA gBattleInterface_Healthbox_Pal
#define BATTLE_HEALTHBAR_PAL_DATA gBattleInterface_Healthbar_Pal
#define BATTLE_HEALTHBOX_SINGLES_PLAYER_GFX gHealthboxSinglesPlayerGfx
#define BATTLE_HEALTHBOX_SINGLES_OPPONENT_GFX gHealthboxSinglesOpponentGfx
#define BATTLE_HEALTHBOX_DOUBLES_PLAYER_GFX gHealthboxDoublesPlayerGfx
#define BATTLE_HEALTHBOX_DOUBLES_OPPONENT_GFX gHealthboxDoublesOpponentGfx
#define BATTLE_HEALTHBOX_SAFARI_GFX gHealthboxSafariGfx
#define BATTLE_HP_NUMBERS_GFX gInterfaceGfx_HPNumbers
#endif

static bool8 IsBattleStartupSpriteIdValid(u8 spriteId)
{
    return spriteId < MAX_SPRITES;
}

static bool8 IsBattleRuntimeSpriteIdValid(u8 spriteId)
{
...
{
    .data = BATTLE_HEALTHBOX_SINGLES_PLAYER_GFX,
    .size = 0x1000,
...
{
    .data = BATTLE_HEALTHBOX_SINGLES_OPPONENT_GFX,
    .size = 0x1000,
...
{
    {
        .data = BATTLE_HEALTHBOX_DOUBLES_PLAYER_GFX,
        .size = 0x800,
        .tag = TAG_HEALTHBOX_PLAYER1_TILE,
    },
    {
...
{
    {
        .data = BATTLE_HEALTHBOX_DOUBLES_OPPONENT_GFX,
        .size = 0x800,
        .tag = TAG_HEALTHBOX_OPPONENT1_TILE,
    },
    {
...
{
    .data = BATTLE_HEALTHBOX_SAFARI_GFX,
    .size = 0x1000,
...
{
    {
        .data = BATTLE_HEALTHBOX_PAL_DATA,
        .tag = TAG_HEALTHBOX_PAL,
    },
    {
        .data = BATTLE_HEALTHBAR_PAL_DATA,
...
#ifdef PORTABLE
    const u32 *picData = GetPortableTrainerFrontPicData(frontPicId);
    const u32 *paletteData = GetPortableTrainerFrontPaletteData(frontPicId);
    void *buffer;
...
bool8 BattleLoadAllHealthBoxesGfx(u8 state)
{
    ...
    if (state)
    {
        ...
        if (IndexOfSpritePaletteTag(sSpritePalettes_HealthBoxHealthBar[0].tag) == 0xFF)
            LoadSpritePalette(&sSpritePalettes_HealthBoxHealthBar[0]);
        if (IndexOfSpritePaletteTag(sSpritePalettes_HealthBoxHealthBar[1].tag) == 0xFF)
            LoadSpritePalette(&sSpritePalettes_HealthBoxHealthBar[1]);
        ...
        if (!IsDoubleBattle())
        {
            ...
            {
                if (GetSpriteTileStartByTag(sSpriteSheet_SafariHealthbox.tag) == 0xFFFF)
                    LoadCompressedSpriteSheetUsingHeap(&sSpriteSheet_SafariHealthbox);
            }
            else if (GetSpriteTileStartByTag(sSpriteSheet_SinglesPlayerHealthbox.tag) == 0xFFFF)
            {
                LoadCompressedSpriteSheetUsingHeap(&sSpriteSheet_SinglesPlayerHealthbox);
            }

            if (GetSpriteTileStartByTag(sSpriteSheet_SinglesOpponentHealthbox.tag) == 0xFFFF)
                LoadCompressedSpriteSheetUsingHeap(&sSpriteSheet_SinglesOpponentHealthbox);
            numberOfBattlers = 2;
        }
        else
        {
            if (GetSpriteTileStartByTag(sSpriteSheets_DoublesPlayerHealthbox[0].tag) == 0xFFFF)
                LoadCompressedSpriteSheetUsingHeap(&sSpriteSheets_DoublesPlayerHealthbox[0]);
            if (GetSpriteTileStartByTag(sSpriteSheets_DoublesPlayerHealthbox[1].tag) == 0xFFFF)
                LoadCompressedSpriteSheetUsingHeap(&sSpriteSheets_DoublesPlayerHealthbox[1]);
            if (GetSpriteTileStartByTag(sSpriteSheets_DoublesOpponentHealthbox[0].tag) == 0xFFFF)
                LoadCompressedSpriteSheetUsingHeap(&sSpriteSheets_DoublesOpponentHealthbox[0]);
            if (GetSpriteTileStartByTag(sSpriteSheets_DoublesOpponentHealthbox[1].tag) == 0xFFFF)
                LoadCompressedSpriteSheetUsingHeap(&sSpriteSheets_DoublesOpponentHealthbox[1]);
            numberOfBattlers = MAX_BATTLERS_COUNT;
        }
        ...
        for (i = 0; i < numberOfBattlers; ++i)
        {
            if (GetSpriteTileStartByTag(sSpriteSheets_HealthBar[gBattlerPositions[i]].tag) == 0xFFFF)
                LoadCompressedSpriteSheetUsingHeap(&sSpriteSheets_HealthBar[gBattlerPositions[i]]);
        }
...
{
    LZDecompressWram(BATTLE_HP_NUMBERS_GFX, gMonSpritesGfxPtr->barFontGfx);
}
...
bool8 BattleInitAllSprites(u8 *state, u8 *battlerId)
{
...
#ifdef PORTABLE
    TraceBattleSpriteInit("BattleSprites: init state=%u battler=%u", *state, *battlerId);
    while (!retVal)
    {
#endif
    ...
    case 1:
        ...
        {
#ifdef PORTABLE
            TraceBattleSpriteInit("BattleSprites: load healthbox gfx battler=%u continue", *battlerId);
#endif
            ++*battlerId;
        }
        else
        {
#ifdef PORTABLE
            TraceBattleSpriteInit("BattleSprites: healthbox gfx complete battler=%u", *battlerId);
#endif
            *battlerId = 0;
            ++*state;
        }
    ...
    case 3:
#ifdef PORTABLE
        for (; *battlerId < gBattlersCount; ++*battlerId)
        {
            ...
            TraceBattleSpriteInit("BattleSprites: created healthbox battler=%u sprite=%u", *battlerId, gHealthboxSpriteIds[*battlerId]);
            if (!IsBattleStartupSpriteIdValid(gHealthboxSpriteIds[*battlerId]))
                gHealthboxSpriteIds[*battlerId] = MAX_SPRITES;
        }
        *battlerId = 0;
        ++*state;
        break;
        ...
        {
#ifdef PORTABLE
            TraceBattleSpriteInit("BattleSprites: invalid healthbox sprite battler=%u", *battlerId);
#endif
            retVal = TRUE;
            break;
        }
    ...
    case 4:
        ...
        for (; *battlerId < gBattlersCount; ++*battlerId)
        {
            if (!IsBattleStartupSpriteIdValid(gHealthboxSpriteIds[*battlerId]))
            {
                TraceBattleSpriteInit("BattleSprites: skip invalid healthbox coords battler=%u", *battlerId);
                continue;
            }
            InitBattlerHealthboxCoords(*battlerId);
            TraceBattleSpriteInit("BattleSprites: init coords battler=%u sprite=%u pos=%u", *battlerId, gHealthboxSpriteIds[*battlerId], gBattlerPositions[*battlerId]);
    ...
    case 5:
        ...
        for (; *battlerId < gBattlersCount; ++*battlerId)
        {
            TraceBattleSpriteInit("BattleSprites: update attributes battler=%u sprite=%u side=%u partyIndex=%u", *battlerId, gHealthboxSpriteIds[*battlerId], GetBattlerSide(*battlerId), gBattlerPartyIndexes[*battlerId]);
            if (!IsBattleStartupSpriteIdValid(gHealthboxSpriteIds[*battlerId]))
            {
                TraceBattleSpriteInit("BattleSprites: skip invalid healthbox attrs battler=%u", *battlerId);
                continue;
            }
    ...
    case 6:
#ifdef PORTABLE
        TraceBattleSpriteInit("BattleSprites: load shadows");
#endif
        LoadAndCreateEnemyShadowSprites();
#ifdef PORTABLE
        TraceBattleSpriteInit("BattleSprites: post-load-shadows");
#endif
        BufferBattlePartyCurrentOrder();
#ifdef PORTABLE
        TraceBattleSpriteInit("BattleSprites: post-buffer-party-order");
#endif
        retVal = TRUE;
        break;
    }
#ifdef PORTABLE
        TraceBattleSpriteInit("BattleSprites: return ret=%u state=%u battler=%u", retVal, *state, *battlerId);
    }
#endif
    return retVal;
}
...
void CopyAllBattleSpritesInvisibilities(void)
{
    ...
    for (i = 0; i < gBattlersCount; ++i)
        if (IsBattleRuntimeSpriteIdValid(gBattlerSpriteIds[i]))
            gBattleSpritesDataPtr->battlerData[i].invisible = gSprites[gBattlerSpriteIds[i]].invisible;
}

void CopyBattleSpriteInvisibility(u8 battlerId)
{
    if (IsBattleRuntimeSpriteIdValid(gBattlerSpriteIds[battlerId]))
...
#ifdef PORTABLE
    TraceBattleSpriteInit("BattleSprites: shadow pre-loadsheet");
#endif
    LoadCompressedSpriteSheetUsingHeap(&gSpriteSheet_EnemyShadow);
    battlerId = GetBattlerAtPosition(B_POSITION_OPPONENT_LEFT);
#ifdef PORTABLE
    TraceBattleSpriteInit("BattleSprites: shadow left battler=%u x=%d y=%d", battlerId, GetBattlerSpriteCoord(battlerId, BATTLER_COORD_X), GetBattlerSpriteCoord(battlerId, BATTLER_COORD_Y) + 29);
#endif
...
#ifdef PORTABLE
    TraceBattleSpriteInit("BattleSprites: shadow left sprite=%u", gBattleSpritesDataPtr->healthBoxesData[battlerId].shadowSpriteId);
#endif
    if (!IsBattleStartupSpriteIdValid(gBattleSpritesDataPtr->healthBoxesData[battlerId].shadowSpriteId))
        return;
    gSprites[gBattleSpritesDataPtr->healthBoxesData[battlerId].shadowSpriteId].data[0] = battlerId;
#ifdef PORTABLE
    TraceBattleSpriteInit("BattleSprites: shadow left post-data0 battler=%u", battlerId);
#endif
    if (IsDoubleBattle())
    {
        battlerId = GetBattlerAtPosition(B_POSITION_OPPONENT_RIGHT);
#ifdef PORTABLE
        TraceBattleSpriteInit("BattleSprites: shadow right battler=%u x=%d y=%d", battlerId, GetBattlerSpriteCoord(battlerId, BATTLER_COORD_X), GetBattlerSpriteCoord(battlerId, BATTLER_COORD_Y) + 29);
#endif
        gBattleSpritesDataPtr->healthBoxesData[battlerId].shadowSpriteId = CreateSprite(&gSpriteTemplate_EnemyShadow, GetBattlerSpriteCoord(battlerId, BATTLER_COORD_X), GetBattlerSpriteCoord(battlerId, BATTLER_COORD_Y) + 29, 0xC8);
#ifdef PORTABLE
        TraceBattleSpriteInit("BattleSprites: shadow right sprite=%u", gBattleSpritesDataPtr->healthBoxesData[battlerId].shadowSpriteId);
#endif
        if (!IsBattleStartupSpriteIdValid(gBattleSpritesDataPtr->healthBoxesData[battlerId].shadowSpriteId))
            return;
        gSprites[gBattleSpritesDataPtr->healthBoxesData[battlerId].shadowSpriteId].data[0] = battlerId;
#ifdef PORTABLE
        TraceBattleSpriteInit("BattleSprites: shadow right post-data0 battler=%u", battlerId);
#endif
    }
}
