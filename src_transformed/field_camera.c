...
#ifdef PORTABLE
static bool8 sLoggedDrawWholeMapViewEnter;
static bool8 sLoggedMissingPrimaryMetatiles;
static bool8 sLoggedMissingSecondaryMetatiles;
static bool8 sLoggedPrimaryMetatilePath;
static bool8 sLoggedSecondaryMetatilePath;
#endif
...
{
#ifdef PORTABLE
    if (!sLoggedDrawWholeMapViewEnter)
    {
        firered_runtime_trace_external("DrawWholeMapView: enter");
        sLoggedDrawWholeMapViewEnter = TRUE;
    }
    if (gMapHeader.mapLayout == NULL)
    {
        firered_runtime_trace_external("DrawWholeMapView: mapLayout-null");
        return;
    }
#endif
...
static void DrawMetatileAt(const struct MapLayout *mapLayout, u16 offset, int x, int y)
{
    ...
    if (metatileId < NUM_METATILES_IN_PRIMARY)
    {
        ...
        if (mapLayout->primaryTileset == NULL || mapLayout->primaryTileset->metatiles == NULL)
        {
            ...
            {
                firered_runtime_trace_external("DrawMetatileAt: primary-metatiles-null");
                sLoggedMissingPrimaryMetatiles = TRUE;
            }
            return;
        }
#endif
    ...
    {
#ifdef PORTABLE
        if (!sLoggedSecondaryMetatilePath)
        {
        ...
        if (mapLayout->secondaryTileset == NULL || mapLayout->secondaryTileset->metatiles == NULL)
        {
            ...
            {
                firered_runtime_trace_external("DrawMetatileAt: secondary-metatiles-null");
                sLoggedMissingSecondaryMetatiles = TRUE;
            }
            return;
        }
#endif
...
{
#ifdef PORTABLE
    firered_runtime_trace_external("CrashTrace: CameraUpdate enter");
#endif
    int deltaX;
    int deltaY;
...
{
#ifdef PORTABLE
    firered_runtime_trace_external("CrashTrace: UpdateCameraPanning enter");
#endif
