...
#ifdef PORTABLE
static bool8 ShouldTraceBgTilemapOps(u8 bg)
{
    return bg == 0 || bg == 2;
}
...
static void TraceBgTilemapOp(const char *kind, u8 bg, void *buffer, const char *context)
{
    ...
    if (context != NULL)
        snprintf(trace, sizeof(trace), "%s: bg=%u buf=%p %s", kind, bg, buffer, context);
    else
...
static void MarkDma3BgCopyBusy(u8 cursor)
{
    if (WaitDma3Request(cursor) == -1)
...
#ifdef PORTABLE
    if (bg >= 1 && bg <= 3 && destOffset == 0)
    {
        char buffer[192];
        const u16 *src16 = src;

        snprintf(buffer, sizeof(buffer),
                 "LoadBgVram: bg=%u src=%p size=%u first=%04X second=%04X",
                 bg,
                 src,
                 size,
                 src16 != NULL ? src16[0] : 0,
                 src16 != NULL ? src16[1] : 0);
...
u16 LoadBgTilemap(u8 bg, const void *src, u16 size, u16 destOffset)
{
...
#ifdef PORTABLE
    {
        ...
        snprintf(context, sizeof(context), "fn=LoadBgTilemap src=%p size=%u destOffset=%u", src, size, destOffset);
        TraceBgTilemapOp("BGWrite", bg, GetBgTilemapBuffer(bg), context);
    }
#endif
...
void SetBgTilemapBuffer(u8 bg, void *tilemap)
{
    ...
    {
#ifdef PORTABLE
        TraceBgTilemapOp("BGSet", bg, tilemap, NULL);
#endif
...
void UnsetBgTilemapBuffer(u8 bg)
{
    ...
    {
#ifdef PORTABLE
        TraceBgTilemapOp("BGSet", bg, NULL, "unset=1");
#endif
        sGpuBgConfigs2[bg].tilemap = NULL;
    }
}
...
void CopyToBgTilemapBuffer(u8 bg, const void *src, u16 mode, u16 destOffset)
{
#ifdef PORTABLE
    {
        ...
        snprintf(context, sizeof(context), "fn=CopyToBgTilemapBuffer src=%p mode=%u destOffset=%u", src, mode, destOffset);
        TraceBgTilemapOp("BGWrite", bg, GetBgTilemapBuffer(bg), context);
    }
#endif
...
void CopyBgTilemapBufferToVram(u8 bg)
{
    ...
    {
#ifdef PORTABLE
        TraceBgTilemapOp("BGCopy", bg, GetBgTilemapBuffer(bg), NULL);
        if (bg <= 3)
        {
            char buffer[192];
            ...
            snprintf(buffer, sizeof(buffer),
                     "CopyBgTilemapBufferToVram: bg=%u tilemap=%p first=%04X second=%04X idx330=%04X",
                     bg,
                     tilemap,
                     tilemap != NULL ? tilemap[0] : 0,
                     tilemap != NULL ? tilemap[1] : 0,
                     tilemap != NULL ? tilemap[330] : 0);
            firered_runtime_trace_external(buffer);
            snprintf(buffer, sizeof(buffer),
                     "CopyBgTilemapBufferMeta: bg=%u screenSize=%u charBase=%u mapBase=%u",
                     bg,
                     GetBgAttribute(bg, BG_ATTR_SCREENSIZE),
                     GetBgAttribute(bg, BG_ATTR_CHARBASEINDEX),
                     GetBgAttribute(bg, BG_ATTR_MAPBASEINDEX));
            firered_runtime_trace_external(buffer);
        }
#endif
        switch (GetBgType(bg))
...
void CopyToBgTilemapBufferRect(u8 bg, const void *src, u8 destX, u8 destY, u8 width, u8 height)
{
...
#ifdef PORTABLE
    {
        ...
        snprintf(context, sizeof(context), "fn=CopyToBgTilemapBufferRect src=%p destX=%u destY=%u width=%u height=%u", src, destX, destY, width, height);
        TraceBgTilemapOp("BGWrite", bg, GetBgTilemapBuffer(bg), context);
    }
#endif
...
void CopyRectToBgTilemapBufferRect(u8 bg, const void *src, u8 srcX, u8 srcY, u8 srcWidth, u8 srcHeight, u8 destX, u8 destY, u8 rectWidth, u8 rectHeight, u8 palette1, s16 tileOffset, s16 palette2)
{
...
#ifdef PORTABLE
    {
        ...
        snprintf(context, sizeof(context), "fn=CopyRectToBgTilemapBufferRect src=%p srcX=%u srcY=%u srcW=%u srcH=%u destX=%u destY=%u rectW=%u rectH=%u", src, srcX, srcY, srcWidth, srcHeight, destX, destY, rectWidth, rectHeight);
        TraceBgTilemapOp("BGWrite", bg, GetBgTilemapBuffer(bg), context);
    }
#endif
...
void FillBgTilemapBufferRect_Palette0(u8 bg, u16 tileNum, u8 x, u8 y, u8 width, u8 height)
{
...
#ifdef PORTABLE
    {
        ...
        snprintf(context, sizeof(context), "fn=FillBgTilemapBufferRect_Palette0 tile=%u x=%u y=%u width=%u height=%u", tileNum, x, y, width, height);
        TraceBgTilemapOp("BGWrite", bg, GetBgTilemapBuffer(bg), context);
    }
#endif
...
void WriteSequenceToBgTilemapBuffer(u8 bg, u16 firstTileNum, u8 x, u8 y, u8 width, u8 height, u8 paletteSlot, s16 tileNumDelta)
{
...
#ifdef PORTABLE
    {
        ...
        snprintf(context, sizeof(context), "fn=WriteSequenceToBgTilemapBuffer firstTile=%u x=%u y=%u width=%u height=%u palette=%u delta=%d", firstTileNum, x, y, width, height, paletteSlot, tileNumDelta);
        TraceBgTilemapOp("BGWrite", bg, GetBgTilemapBuffer(bg), context);
    }
#endif
