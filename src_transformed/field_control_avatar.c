...
static bool8 ShouldTraceWarpCheck(void)
{
    unsigned long frame = firered_runtime_get_completed_frame_external();
    return frame >= 7300 && frame <= 8200;
}

static void TraceWarpCheck(const char *fmt, ...)
{
    unsigned long frame;
    char buffer[256];
    va_list args;

    if (!ShouldTraceWarpCheck())
        return;

    frame = firered_runtime_get_completed_frame_external();
    va_start(args, fmt);
...
#ifdef PORTABLE
    TraceWarpCheck("WARPCHK frame=%lu enter posRaw=(%u,%u,%u) posMap=(%u,%u) facing=%u held=%04X dpad=%u heldDir=%u heldDir2=%u took=%u meta=%04X",
                   firered_runtime_get_completed_frame_external(),
                   position.x,
                   position.y,
                   position.elevation,
...
static const u8 *GetInteractedBackgroundEventScript(struct MapPosition *position, u8 metatileBehavior, u8 direction)
{
    ...
    if (signpostType != SIGNPOST_NA)
        MsgSetSignpost();
    gSpecialVar_Facing = direction;
    return ResolveMapEventScriptPointer(bgEvent->bgUnion.script);
}
...
static const u8 *GetSignpostScriptAtMapPosition(struct MapPosition * position)
{
    ...
    if (event->bgUnion.script != NULL)
        return ResolveMapEventScriptPointer(event->bgUnion.script);
    return EventScript_TestSignpostMsg;
}
...
static bool8 TryArrowWarp(struct MapPosition *position, u16 metatileBehavior, u8 direction)
{
    ...
    u16 delay;
#ifdef PORTABLE
    bool8 isArrow = IsArrowWarpMetatileBehavior(metatileBehavior, direction);
    bool8 isStair = IsDirectionalStairWarpMetatileBehavior(metatileBehavior, direction);
    TraceWarpCheck("WARPCHK frame=%lu TryArrowWarp posRaw=(%u,%u,%u) posMap=(%u,%u) dir=%u meta=%04X warpId=%d arrow=%u stair=%u",
                   firered_runtime_get_completed_frame_external(),
                   position->x,
                   position->y,
                   position->elevation,
...
#ifdef PORTABLE
            TraceWarpCheck("WARPCHK frame=%lu TryArrowWarp stair transition started warpId=%d delay=%u",
                           firered_runtime_get_completed_frame_external(),
                           warpEventId,
                           delay);
#endif
...
static bool8 TryStartWarpEventScript(struct MapPosition *position, u16 metatileBehavior)
{
...
#ifdef PORTABLE
    TraceWarpCheck("WARPCHK frame=%lu TryStartWarpEventScript posRaw=(%u,%u,%u) posMap=(%u,%u) meta=%04X warpId=%d isWarp=%u",
                   firered_runtime_get_completed_frame_external(),
                   position->x,
                   position->y,
                   position->elevation,
                   ...
                   metatileBehavior,
                   warpEventId,
                   IsWarpMetatileBehavior(metatileBehavior));
#endif
    ...
    if (warpEventId != -1 && IsWarpMetatileBehavior(metatileBehavior) == TRUE)
    {
        ...
        DoWarp();
#ifdef PORTABLE
        TraceWarpCheck("WARPCHK frame=%lu TryStartWarpEventScript DoWarp warpId=%d",
                       firered_runtime_get_completed_frame_external(),
                       warpEventId);
#endif
        return TRUE;
...
static s8 GetWarpEventAtPosition(struct MapHeader *mapHeader, u16 x, u16 y, u8 elevation)
{
...
#ifdef PORTABLE
    TraceWarpCheck("WARPCHK frame=%lu GetWarpEventAtPosition check x=%u y=%u elev=%u warpCount=%u map=(%u,%u)",
                   firered_runtime_get_completed_frame_external(),
                   x,
                   y,
                   elevation,
    ...
    {
#ifdef PORTABLE
        TraceWarpCheck("WARPCHK frame=%lu warp[%ld]=(x=%d y=%d elev=%u dest=%u,%u id=%u) posMatch=%u elevMatch=%u",
                       firered_runtime_get_completed_frame_external(),
                       (long)i,
                       warpEvent->x,
                       warpEvent->y,
                       ...
                       warpEvent->mapNum,
                       warpEvent->warpId,
                       ((u16)warpEvent->x == x && (u16)warpEvent->y == y),
                       (warpEvent->elevation == elevation || warpEvent->elevation == 0));
#endif
...
#ifdef PORTABLE
    TraceWarpCheck("WARPCHK frame=%lu GetWarpEventAtPosition result=-1", firered_runtime_get_completed_frame_external());
#endif
    return -1;
}
...
static const u8 *TryRunCoordEventScript(const struct CoordEvent *coordEvent)
{
    if (coordEvent != NULL)
    {
        ...
        {
            RunScriptImmediately(ResolveMapEventScriptPointer(coordEvent->script));
            return NULL;
        }
        if (VarGet(coordEvent->trigger) == (u8)coordEvent->index)
            return ResolveMapEventScriptPointer(coordEvent->script);
    }
    return NULL;
}
    ...
    if (MapGridGetMetatileBehaviorAt(x, y) == MB_STRENGTH_BUTTON)
    {
        for (i = 0; i < n; i++)
        {
            if (events[i].x + MAP_OFFSET == x && events[i].y + MAP_OFFSET == y)
            {
                ...
                ScriptContext_SetupScript(ResolveMapEventScriptPointer(events[i].script));
                LockPlayerFieldControls();
            }
        }
    }
