...
#ifdef PORTABLE
/*
 * Map ASCII printable bytes (from C string literals compiled without the GBA
 * charmap preprocessor) to the corresponding GBA character codes so that the
 * GBA font glyph table produces the correct glyphs.  Bytes >= 0x80 are already
 * in GBA encoding (e.g. names from save data) and are returned unchanged.
 */
static const u8 sAsciiToGba[128] = {
    ...
    /* 0x30 */ 0xA1, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8, /* 01234567 */
    /* 0x38 */ 0xA9, 0xAA, 0xF0, 0x36, 0x85, 0x35, 0x86, 0xAC, /* 89:;<=>? */
    /* 0x40 */ 0x00, 0xBB, 0xBC, 0xBD, 0xBE, 0xBF, 0xC0, 0xC1, /* @ABCDEFG */
    /* 0x48 */ 0xC2, 0xC3, 0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, /* HIJKLMNO */
    /* 0x50 */ 0xCA, 0xCB, 0xCC, 0xCD, 0xCE, 0xCF, 0xD0, 0xD1, /* PQRSTUVW */
    /* 0x58 */ 0xD2, 0xD3, 0xD4, 0x00, 0x00, 0x00, 0x00, 0x00, /* XYZ[\]^_ */
    /* 0x60 */ 0x00, 0xD5, 0xD6, 0xD7, 0xD8, 0xD9, 0xDA, 0xDB, /* `abcdefg */
    /* 0x68 */ 0xDC, 0xDD, 0xDE, 0xDF, 0xE0, 0xE1, 0xE2, 0xE3, /* hijklmno */
    /* 0x70 */ 0xE4, 0xE5, 0xE6, 0xE7, 0xE8, 0xE9, 0xEA, 0xEB, /* pqrstuvw */
    /* 0x78 */ 0xEC, 0xED, 0xEE, 0x00, 0x00, 0x00, 0x00, 0x00, /* xyz{|}~. */
};

static u8 portable_ascii_to_gba(u8 c)
{
    if (c < 0x80)
...
static u32 portable_preprocess_battle_str(const u8 *src, u8 *dst, u32 dstSize)
{
    ...
    while (*src != '\0' && d + 2 < dstSize)
    {
        if (*src == '\\')
        {
            if (src[1] == 'p')
            {
                dst[d++] = CHAR_PROMPT_CLEAR;
                src += 2;
                continue;
            }
            if (src[1] == 'l')
            {
                dst[d++] = CHAR_PROMPT_SCROLL;
                src += 2;
                continue;
            }
            if (src[1] == 'n')
...
#define EMIT(b) do { if (d < dstSize - 1) dst[d++] = (u8)(b); } while (0)
    ...
    while (*src != '\0' && *src != EOS)
    {
        ...
        if (c == '\\')
        {
            if (src[1] == 'p')
            {
                EMIT(CHAR_PROMPT_CLEAR);
                src += 2;
                continue;
            }
            if (src[1] == 'l')
            {
                EMIT(CHAR_PROMPT_SCROLL);
                src += 2;
                continue;
            }
            if (src[1] == 'n')
        ...
        else if (c == EXT_CTRL_CODE_BEGIN) /* 0xFC – already-binary control code */
        {
            u8 code, nparams, p;
            EMIT(c);
            src++;
...
static const u8 sText_PlayerPanicked[] = _("{B_PLAYER_NAME} panicked and lost ¥{B_BUFF1}…\xFB… … … …\xFB{B_PLAYER_NAME} whited out!{PAUSE_UNTIL_PRESS}");
static const u8 sText_PlayerWhiteoutAgainstTrainer[] = _("{B_PLAYER_NAME} is out of\nusable POKéMON!\xFBPlayer lost against\n{B_TRAINER1_CLASS} {B_TRAINER1_NAME}!{PAUSE_UNTIL_PRESS}");
static const u8 sText_PlayerPaidAsPrizeMoney[] = _("{B_PLAYER_NAME} paid ¥{B_BUFF1} as the prize\nmoney…\xFB… … … …\xFB{B_PLAYER_NAME} whited out!{PAUSE_UNTIL_PRESS}");
static const u8 sText_PlayerWhiteout2[] = _("{B_PLAYER_NAME} whited out!{PAUSE_UNTIL_PRESS}");
static const u8 sText_PreventsEscape[] = _("{B_SCR_ACTIVE_NAME_WITH_PREFIX} prevents\nescape with {B_SCR_ACTIVE_ABILITY}!\xFB");
static const u8 sText_CantEscape2[] = _("Can't escape!\xFB");
...
static const u8 sText_PkmnsXPreventsYsZ[] = _("{B_ATK_NAME_WITH_PREFIX}'s {B_ATK_ABILITY}\nprevents {B_DEF_NAME_WITH_PREFIX}'s\xFA{B_DEF_ABILITY} from working!");
...
static const u8 sText_123Poof[] = _("{PAUSE 32}1, {PAUSE 15}2, and{PAUSE 15}… {PAUSE 15}… {PAUSE 15}… {PAUSE 15}{PLAY_SE SE_BALL_BOUNCE_1}Poof!\xFB");
static const u8 sText_AndEllipsis[] = _("And…\xFB");
...
static const u8 sText_NoRunningFromTrainers[] = _("No! There's no running\nfrom a TRAINER battle!\xFB");
static const u8 sText_CantEscape[] = _("Can't escape!\xFB");
...
static const u8 sText_OutOfSafariBalls[] = _("{PLAY_SE SE_DING_DONG}ANNOUNCER: You're out of\nSAFARI BALLS! Game over!\xFB");
static const u8 sText_WildPkmnAppeared[] = _("Wild {B_OPPONENT_MON1_NAME} appeared!\xFB");
static const u8 sText_WildPkmnAppeared2[] = _("Wild {B_OPPONENT_MON1_NAME} appeared!\xFB");
static const u8 sText_WildPkmnAppearedPause[] = _("Wild {B_OPPONENT_MON1_NAME} appeared!{PAUSE 127}");
static const u8 sText_TwoWildPkmnAppeared[] = _("Wild {B_OPPONENT_MON1_NAME} and\n{B_OPPONENT_MON2_NAME} appeared!\xFB");
static const u8 sText_GhostAppearedCantId[] = _("The GHOST appeared!\xFBDarn!\nThe GHOST can't be ID'd!\xFB");
static const u8 sText_TheGhostAppeared[] = _("The GHOST appeared!\xFB");
static const u8 sText_SilphScopeUnveil[] = _("SILPH SCOPE unveiled the GHOST's\nidentity!");
static const u8 sText_TheGhostWas[] = _("The GHOST was MAROWAK!\xFB\n");
static const u8 sText_Trainer1WantsToBattle[] = _("{B_TRAINER1_CLASS} {B_TRAINER1_NAME}\nwould like to battle!\xFB");
...
static const u8 sText_GotchaPkmnCaught[] = _("Gotcha!\n{B_OPPONENT_MON1_NAME} was caught!{WAIT_SE}{PLAY_BGM MUS_CAUGHT}\xFB");
...
static const u8 sText_ItemAllowsOnlyYMove[] = _("{B_LAST_ITEM}'s effect allows only\n{B_CURRENT_MOVE} to be used!\xFB");
static const u8 sText_PkmnHungOnWithX[] = _("{B_DEF_NAME_WITH_PREFIX} hung on\nusing its {B_LAST_ITEM}!");
const u8 gText_EmptyString3[] = { EOS };
static const u8 sText_PlayedFluteCatchyTune[] = _("{B_PLAYER_NAME} played the {B_LAST_ITEM}.\xFBNow, that's a catchy tune!");
...
const u8 gText_ForPetesSake[] = _("OAK: Oh, for Pete's sake…\nSo pushy, as always.\xFB{B_PLAYER_NAME}.\xFBYou've never had a POKéMON battle\nbefore, have you?\xFBA POKéMON battle is when TRAINERS\npit their POKéMON against each\xFAother.\xFB");
const u8 gText_TheTrainerThat[] = _("The TRAINER that makes the other\nTRAINER's POKéMON faint by lowering\xFAtheir HP to “0,” wins.\xFB");
const u8 gText_TryBattling[] = _("But rather than talking about it,\nyou'll learn more from experience.\xFBTry battling and see for yourself.\xFB");
const u8 gText_InflictingDamageIsKey[] = _("OAK: Inflicting damage on the foe\nis the key to any battle.\xFB");
const u8 gText_LoweringStats[] = _("OAK: Lowering the foe's stats\nwill put you at an advantage.\xFB");
const u8 gText_KeepAnEyeOnHP[] = _("OAK: Keep your eyes on your\nPOKéMON's HP.\xFBIt will faint if the HP drops to\n“0.”\xFB");
const u8 gText_OakNoRunningFromATrainer[] = _("OAK: No! There's no running away\nfrom a TRAINER POKéMON battle!\xFB");
const u8 gText_WinEarnsPrizeMoney[] = _("OAK: Hm! Excellent!\xFBIf you win, you earn prize money,\nand your POKéMON will grow!\xFBBattle other TRAINERS and make\nyour POKéMON strong!\xFB");
const u8 gText_HowDissapointing[] = _("OAK: Hm…\nHow disappointing…\xFBIf you win, you earn prize money,\nand your POKéMON grow.\xFBBut if you lose, {B_PLAYER_NAME}, you end\nup paying prize money…\xFBHowever, since you had no warning\nthis time, I'll pay for you.\xFBBut things won't be this way once\nyou step outside these doors.\xFBThat's why you must strengthen your\nPOKéMON by battling wild POKéMON.\xFB");
...
const u8 gText_CongratsPkmnEvolved[] = _("Congratulations! Your {STR_VAR_1}\nevolved into {STR_VAR_2}!{WAIT_SE}\xFB");
const u8 gText_PkmnStoppedEvolving[] = _("Huh? {STR_VAR_1}\nstopped evolving!\xFB");
const u8 gText_EllipsisQuestionMark[] = _("……?\xFB");
...
u32 BattleStringExpandPlaceholders(const u8 *src, u8 *dst)
{
...
#endif
    {
        ...
        else
        {
            if (dstId + 1 < dstCapacity)
#ifdef PORTABLE
                dst[dstId++] = portable_ascii_to_gba(*src);
#else
                dst[dstId++] = *src;
#endif
        }
