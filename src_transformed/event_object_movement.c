...
static u8 sPortableHeldMoveTraceCount;

static void TraceHeldMove(const char *fmt, ...)
{
    char buffer[256];
    va_list args;

    if (sPortableHeldMoveTraceCount >= 96)
        return;
...
#ifdef PORTABLE
#include "object_event_graphics_portable_assets.h"
#else
...
#ifdef PORTABLE
    script = ResolveMapEventScriptPointer(script);
#endif
    return script;
}
...
#ifdef PORTABLE
    TraceHeldMove("HeldMove: set local=%u sprite=%u action=%u step=%d tStep=%d speed=%d x=%d y=%d", objectEvent->localId, objectEvent->spriteId, movementActionId, gSprites[objectEvent->spriteId].data[2], gSprites[objectEvent->spriteId].data[5], gSprites[objectEvent->spriteId].data[4], gSprites[objectEvent->spriteId].x, gSprites[objectEvent->spriteId].y);
#endif
    return FALSE;
}
...
#ifdef PORTABLE
    if (heldMovementStatus == 0 && objectEvent->heldMovementActive)
        TraceHeldMove("HeldMove: waiting local=%u sprite=%u action=%u step=%d finished=%u tStep=%d speed=%d x=%d y=%d", objectEvent->localId, objectEvent->spriteId, objectEvent->movementActionId, gSprites[objectEvent->spriteId].data[2], objectEvent->heldMovementFinished, gSprites[objectEvent->spriteId].data[5], gSprites[objectEvent->spriteId].data[4], gSprites[objectEvent->spriteId].x, gSprites[objectEvent->spriteId].y);
#endif
...
{
#ifdef PORTABLE
    TraceHeldMove("HeldMove: exec local=%u sprite=%u action=%u step=%d tStep=%d speed=%d x=%d y=%d", objectEvent->localId, objectEvent->spriteId, objectEvent->movementActionId, sprite->data[2], sprite->data[5], sprite->data[4], sprite->x, sprite->y);
#endif
    if (sMovementActionFuncs[objectEvent->movementActionId][sprite->data[2]](objectEvent, sprite))
    {
        objectEvent->heldMovementFinished = TRUE;
#ifdef PORTABLE
        TraceHeldMove("HeldMove: finished local=%u sprite=%u action=%u step=%d tStep=%d speed=%d x=%d y=%d", objectEvent->localId, objectEvent->spriteId, objectEvent->movementActionId, sprite->data[2], sprite->data[5], sprite->data[4], sprite->x, sprite->y);
#endif
    }
}
