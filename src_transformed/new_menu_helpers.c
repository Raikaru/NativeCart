...
static EWRAM_DATA u8 sStartMenuWindowId = {0xFF};
#ifdef PORTABLE
...
#ifdef PORTABLE
static u8 StoreHeapPtrForTask(void *ptr);
static void *GetHeapPtrForTask(u8 slot);
#endif

#ifdef PORTABLE
static u8 StoreHeapPtrForTask(void *ptr)
{
    u8 slot = sHeapPtrSlot;

    sHeapPtrTable[slot] = ptr;
    sHeapPtrSlot = (sHeapPtrSlot + 1) % NELEMS(sHeapPtrTable);
    return slot;
}

static void *GetHeapPtrForTask(u8 slot)
{
    if (slot >= NELEMS(sHeapPtrTable))
        return NULL;
    return sHeapPtrTable[slot];
}
#endif
...
{
#ifdef PORTABLE
    firered_runtime_trace_external("CrashTrace: DoScheduledBgTilemapCopiesToVram enter");
#endif
...
{
    int i;
    bool8 dmaBusy = IsDma3ManagerBusyWithBgCopy();

    if (!dmaBusy)
    {
#ifdef PORTABLE
        firered_runtime_trace_external("FreeTempTileDataBuffersIfPossible: dma_busy=0 return=0");
#endif
    ...
    {
#ifdef PORTABLE
        firered_runtime_trace_external("FreeTempTileDataBuffersIfPossible: dma_busy=1 return=1");
#endif
        return TRUE;
    }
}
...
static void TaskFreeBufAfterCopyingTileDataToVram(u8 taskId)
{
    ...
    {
#ifdef PORTABLE
        u8 slot = GetWordTaskArg(taskId, 1);
        Free(GetHeapPtrForTask(slot));
        sHeapPtrTable[slot] = NULL;
#else
...
#ifdef PORTABLE
    if ((fontId == FONT_NORMAL || fontId == FONT_NORMAL_COPY_1)
        && (attributeId == FONTATTR_LETTER_SPACING || attributeId == FONTATTR_LINE_SPACING))
    {
        return 1;
    }
#endif
